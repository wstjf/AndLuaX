...
a=1
b=2
mGestureDetector=GestureDetector(activity,OnGestureListener{
  ...
  if e.getAction() == MotionEvent.ACTION_DOWN then
   ...
   elseif e.getAction() == MotionEvent.ACTION_MOVE then
    ...
    if zoomX then
     ...
     elseif zoomY then
      ...
      v.TranslationX=(vx+(rx-lastX))
      v.TranslationY=(vx+(rx-lastX))
    end
   ...
   elseif e.getAction() == MotionEvent.ACTION_UP then
    if (rx-lastX)^2<100 and (ry-lastY)^2<100 then
      ...
      curr.TranslationX=to(vx+(rx-lastX))
      curr.TranslationY=to(vy+(ry-lastY))
      if zoomX then
...
ns={
  "小部件","Widget 检查视图","Check view 适配器视图","Adapter view 高级控件","Advanced Widget 布局","Layout 高级布局","Advanced Layout 附加控件"
}
