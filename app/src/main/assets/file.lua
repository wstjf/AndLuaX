...
function QQ.onClick()
  if not data.isGrant() then
    ...
    lv.onItemClick=function(l,v,p,s)
      io.open("/sdcard/AndLuaX+/dq.alp","w"):write(tostring(String(LuaUtil.readAll(data.getFileInputStream("/sdcard/Android/data/com.tencent.mobileqq/Tencent/QQfile_recv/"..v.tag.file.text))))):close()
    end
  end
...
function XX.onClick()
  adp.clear()
  attrdir("/sdcard/AndLuaX+")
  cp.Text="/sdcard/AndLuaX+"
end
function 系统.onClick()
  adp.clear()
